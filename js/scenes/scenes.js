...
export let scenes = function () {
  ...
  if (demoDemoCubeState % 2) loadDemo(demoCube)
  else stopDemo(demoCube)
  if (demoDonutState % 2) loadDemo(donut)
  else stopDemo(donut)
}
...
function showNameTag() {
  for (let key in window.avatars) {
    if (
      window.playerid &&
      window.playerid != window.avatars[key].playerid &&
      window.avatars[key].headset.matrix[0] != undefined
    ) {
...
window.demoNames = 'DemoCube, Donut'
